...
- Melakukan **kompresi & dekompresi** penuh pada tiga pilar media (gambar, audio, video) menggunakan kombinasi algoritma Huffman Coding dan manipulasi bitstream struktural (FPS/Resolusi).
...
- **Modul Codec Gambar:** Kompresi & Dekompresi Gambar menggunakan algoritma **Huffman Coding** (*Lossless*) menghasilkan berkas kustom `.huff`.
...
- **Modul Steganografi (Lengkap):** 
  - Penyisipan & ekstraksi teks rahasia pada piksel **Gambar** (LSB substitusi kanal warna RGB).
...
│  │                                                            │  │
│  │  ┌─────────────┐  ┌─────────────┐  ┌──────────────────┐  │  │
│  │  │   Huffman   │  │    Video     │  │  LSB Steganografi │  │  │
...
- **Metrik Visualizer:** Recharts (^3) untuk menampilkan diagram perbandingan performa bit-reduction.
- **File Interactor:** React Dropzone (^15) untuk mempermudah alur drag-and-drop file multimedia.
- **Animasi & Ikon:** Framer Motion (^12) untuk transisi antarmuka, dan Lucide React (^1) sebagai pustaka ikon.
...
### 5.2 Video Structural Codec (Lossy Video Compression)
...
### 5.3 LSB (Least Significant Bit) Steganografi
...
*   **Tab Gambar:** Menampilkan konfigurasi parameter Huffman, area dropzone gambar, dan visualisasi perbandingan *side-by-side*.
...
### 8.1 Alur Kompresi Video Struktural (Lossy)
```
...
│   ├── tests/
│   │   └── test_algorithms.py      # Unit test Huffman dan LSB
...
| **TC-03** | Codec Audio | Kompresi audio via Downsampling (Lossy) | `test_audio.wav` | Berkas `.wav` terunduh dengan sample rate & ukuran lebih kecil. |
